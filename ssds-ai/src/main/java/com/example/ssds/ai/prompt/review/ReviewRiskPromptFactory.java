...
public class ReviewRiskPromptFactory {
    public static final String PROMPT_VERSION = "review-risk-v4";
    ...
    public String retryInstruction(String validationCode) {
        return """
                ...
                3. sentiment 只能是 POSITIVE、NEUTRAL、NEGATIVE；只有 NEGATIVE 可且必須設定 riskTopic，其餘必須為 null。
                4. topicStatistics 必須各輸出一次 QUALITY、FOOD_SAFETY、SHIPPING_DAMAGE、PRICE、OTHER；ratio 必須與逐筆負評分類一致。
                5. ratio 必須是 0 到 1 的 JSON number，severity 只能是 LOW、MEDIUM、HIGH。
                6. 只輸出 JSON，不得加上 Markdown、說明文字、分數、權重或額外欄位。
