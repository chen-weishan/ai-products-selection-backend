...
public class TrendInterpreterPromptFactory {
    ...
    public String retryInstruction(String validationCode) {
        return """
                修正要求：上一次輸出未通過 TrendInterpreter Schema（%s），請重新輸出完整 JSON：
                1. 根物件只能包含 stage、stageWeeks、estimatedLifespanDays，三個欄位都不可省略。
                2. stage 只能是 RISING、PLATEAU、DECLINING；stageWeeks 與 estimatedLifespanDays 必須是整數。
                3. 三個欄位必須完整複製 INPUT_JSON.allowedOutputs 中同一筆資料，不得混搭、換算或產生新數字。
                4. 只輸出 JSON，不得加上 Markdown、理由、說明文字或額外欄位。
