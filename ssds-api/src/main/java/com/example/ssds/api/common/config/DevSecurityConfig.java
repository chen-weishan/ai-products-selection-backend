...
//    }
	@Bean
	SecurityFilterChain devFilterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.disable()) // 徹底掐死 CSRF
				.cors(cors -> cors.disable()) // 徹底掐死 CORS 攔截
				.authorizeHttpRequests(auth -> auth.requestMatchers("/api/auth/**").permitAll()
						.requestMatchers("/api/v1/auth/**").permitAll().anyRequest().authenticated()) // 👈 這裡的右括號要跟
																										// authorizeHttpRequests
																										// 對齊
				.httpBasic(Customizer.withDefaults()); // 👈 連鎖到這裡結尾加分號
