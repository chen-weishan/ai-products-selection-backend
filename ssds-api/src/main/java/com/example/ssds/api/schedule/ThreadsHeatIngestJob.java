...
/**
 * 每日採集 Threads 關鍵字熱度，落地到 heat_reading（關鍵字級）。
 * target 直接是 trend_keyword.keyword，不需要 hashtag→品類對照
 * （對照 InstagramHeatIngestJob）。只寫入 raw_value，百分位化由
...
public class ThreadsHeatIngestJob {
    ...
    private final ThreadsHeatSourceAdapter threadsAdapter;

    public ThreadsHeatIngestJob(
            ...
            ThreadsHeatSourceAdapter threadsAdapter) {
    ...
    public void run() {
        ...
        if (!source.isEnabled()) {
            log.info("THREADS 來源已停用（enabled=false），略過採集。");
            return;
        }
        ...
        try {
            points = threadsAdapter.fetch(keywordTexts, today);
        } catch (Exception e) {
            log.error("Threads 熱度採集整批失敗，來源標記為 UNAVAILABLE", e);
