 ...
 *
 * <p>
 * AC-16-2：B 軌不產生選品分數，排序依據是<b>時效落差</b>而非熱度 ——
 * 熱度最高但來不及的品項排在前面不具意義。
 *
 * <p>
 * 時效落差是<b>否決條件</b>而非加權因子（FR-16-1 設計理由）：
 * 若寫成扣分，高熱度會把它蓋過去，但「來不及」是不可交易的事實。
 *
 * <p>
 * <b>主關聯是品項而非關鍵字</b>（v3.0 §7.2.9 裁決）。v2.0 綁 keyword_id
 * 與 {@code product.track_type} 的模型互斥，AC-16-5「成案轉軌後熱度資料完整
 * 保留」沒有實作路徑；改綁品項後，轉軌只需改 track_type，熱度、標記與關鍵字
 * 關聯全部自然保留。
 *
 * <p>
 * 狀態不存於本實體，一律以 {@code product.sourcingStatus} 為準（§7.2.9）。
 */
@Getter
@Setter
@Builder
...
public class SourcingCandidate extends BaseAuditEntity {
     ...
     *
     * <p>
     * 允許與 {@code product_keyword} 的現況不一致，也允許為 null
