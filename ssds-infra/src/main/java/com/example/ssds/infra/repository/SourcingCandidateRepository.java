...
public interface SourcingCandidateRepository extends JpaRepository<SourcingCandidate, Long> {
  ...
  @Query("""
      SELECT sc FROM SourcingCandidate sc
      JOIN FETCH sc.product p
      WHERE p.trackType = 'B'
        AND p.sourcingStatus != 'REJECTED'
      ORDER BY sc.timeGapDays ASC NULLS LAST
      """)
  List<SourcingCandidate> findPriorityListWithLimit(@Param("limit") int limit);
