...
-- 資料庫端的 partial unique index 保證同時只有一筆 ACTIVE。

INSERT INTO weight_version (id, version_no, name, status, effective_from,
...
--   季節導向型     20%     25%    20%      5%    30%
--
-- 註：§5.2.1 另列有 heat_volume 與 price_fit 兩個加分因子，但 FR-08 的
-- 初始權重表未分配權重給它們（四組皆只列五個因子）。此處忠實照表建立，
-- v3 草稿才依校準結果納入 heat_volume —— 這也是 FR-15 校準機制存在的意義。
...
INSERT INTO seed_bonus (product_id, factor_code, raw_value, normalized_value) VALUES
  ...
  (101, 'HEAT_SLOPE', 3.4012, 96.00), (101, 'MARGIN', 0.5194, 76.00),
  (101, 'CONVERSION', 0.0412, 81.00), (101, 'FESTIVAL', 0.8800, 88.00),
  (101, 'CLIMATE', 0.9300, 93.00),
  -- 102 韓式草莓夾心年糕：冬季限定
  (102, 'HEAT_SLOPE', 1.2400, 78.00), (102, 'MARGIN', 0.5152, 74.00),
  (102, 'CONVERSION', 0.0350, 70.00), (102, 'FESTIVAL', 0.3000, 30.00),
  (102, 'CLIMATE', 0.8800, 88.00),
  -- 103 泰式打拋豬即食調理包
  (103, 'HEAT_SLOPE', 0.1200, 52.00), (103, 'MARGIN', 0.4966, 66.00),
  (103, 'CONVERSION', 0.0388, 74.00), (103, 'FESTIVAL', 0.2000, 20.00),
  (103, 'CLIMATE', 0.5500, 55.00),
  -- 104 冷凍舒芙蕾鬆餅：分數不低，但冷鏈與負評把它壓下去
  (104, 'HEAT_SLOPE', 0.3100, 68.00), (104, 'MARGIN', 0.5377, 90.00),
  (104, 'CONVERSION', 0.0455, 88.00), (104, 'FESTIVAL', 0.4000, 40.00),
  (104, 'CLIMATE', 0.7200, 72.00),
  -- 105 手作黑糖珍珠鮮奶茶包：常態補貨型的模範生
  (105, 'HEAT_SLOPE', 0.2200, 72.00), (105, 'MARGIN', 0.6067, 95.00),
  (105, 'CONVERSION', 0.0520, 96.00), (105, 'FESTIVAL', 0.3000, 30.00),
  (105, 'CLIMATE', 0.7000, 70.00),
  -- 106 冰萃冷泡茶禮盒
  (106, 'HEAT_SLOPE', 0.6800, 74.00), (106, 'MARGIN', 0.5263, 78.00),
  (106, 'CONVERSION', 0.0301, 64.00), (106, 'FESTIVAL', 0.3500, 35.00),
  (106, 'CLIMATE', 0.9000, 90.00),
  -- 107 中秋蛋黃酥禮盒：節慶時間窗正值黃金備貨期
  (107, 'HEAT_SLOPE', 1.8500, 88.00), (107, 'MARGIN', 0.5417, 86.00),
  (107, 'CONVERSION', 0.0430, 84.00), (107, 'FESTIVAL', 0.9800, 98.00),
  (107, 'CLIMATE', 0.6200, 62.00),
  -- 108 減醣蒟蒻麵
  (108, 'HEAT_SLOPE', 0.4400, 66.00), (108, 'MARGIN', 0.5789, 88.00),
  (108, 'CONVERSION', 0.0244, 52.00), (108, 'FESTIVAL', 0.2200, 22.00),
  (108, 'CLIMATE', 0.5600, 56.00),
  -- 109 天然酵素洗衣精補充包
  (109, 'HEAT_SLOPE', 0.0300, 48.00), (109, 'MARGIN', 0.5109, 75.00),
  (109, 'CONVERSION', 0.0610, 90.00), (109, 'FESTIVAL', 0.1500, 15.00),
  (109, 'CLIMATE', 0.5000, 50.00),
  -- 110 竹漿本色抽取衛生紙：轉換率極高但毛利率偏低
  (110, 'HEAT_SLOPE', 0.0100, 45.00), (110, 'MARGIN', 0.4150, 58.00),
  (110, 'CONVERSION', 0.0702, 94.00), (110, 'FESTIVAL', 0.1200, 12.00),
  (110, 'CLIMATE', 0.4800, 48.00),
  -- 111 胺基酸溫和洗面乳
  (111, 'HEAT_SLOPE', 0.1500, 62.00), (111, 'MARGIN', 0.5156, 88.00),
  (111, 'CONVERSION', 0.0566, 92.00), (111, 'FESTIVAL', 0.2000, 20.00),
  (111, 'CLIMATE', 0.5500, 55.00),
  -- 113 霧面絲絨唇釉：CONVERSION 為缺值填補（退回全品類百分位，§5.3.1）
  (113, 'HEAT_SLOPE', 2.6000, 92.00), (113, 'MARGIN', 0.6466, 95.00),
  (113, 'FESTIVAL', 0.3000, 30.00),  (113, 'CLIMATE', 0.6000, 60.00),
  -- 115 隨行果汁攪拌杯
  (115, 'HEAT_SLOPE', 0.1800, 58.00), (115, 'MARGIN', 0.5362, 80.00),
  (115, 'CONVERSION', 0.0201, 45.00), (115, 'FESTIVAL', 0.2000, 20.00),
  (115, 'CLIMATE', 0.8600, 86.00),
  -- 116 泰式手標奶茶粉：已淘汰
  (116, 'HEAT_SLOPE', -0.0800, 30.00), (116, 'MARGIN', 0.5630, 82.00),
  (116, 'CONVERSION', 0.0166, 40.00), (116, 'FESTIVAL', 0.1500, 15.00),
...
SELECT m.score_id,
       ...
       m.scene_type,
       agg.bonus,          -- base_score：v1.0 欄位名，語意同 bonus_subtotal
       agg.bonus,
       agg.penalty,        -- risk_penalty：v1.0 欄位名，語意同 penalty_subtotal
       agg.penalty,
       GREATEST(0, agg.bonus - agg.penalty),
       CASE
           -- §5.6 硬規則：扣分達 20 以上者分級最高只給 B
           WHEN GREATEST(0, agg.bonus - agg.penalty) >= wv.grade_a_threshold
                AND agg.penalty < 20                                THEN 'A'
           WHEN GREATEST(0, agg.bonus - agg.penalty) >= wv.grade_b_threshold THEN 'B'
           ELSE 'C'
       END,
       m.confidence,
       now() - interval '2 days'
FROM seed_score_meta m
         JOIN weight_version wv ON wv.id = 2
         JOIN LATERAL (
...
SELECT m.score_id, p.factor_code, p.raw_value, NULL, NULL,
       p.penalty_value, FALSE, TRUE, TRUE
FROM seed_penalty p
...
SELECT 100 + ROW_NUMBER() OVER (ORDER BY s.id, w.wk),
       ...
       s.scene_type,
       adj.bonus, adj.bonus, s.penalty_subtotal, s.penalty_subtotal,
...
INSERT INTO ai_insight (product_id, insight_type, content_json, model, prompt_version,
                        source_review_count, prompt_tokens, completion_tokens, cost_usd,
  ...
  (101, 'SELLING_POINT',
   ...
   'meta-llama/llama-3.3-70b-instruct:free', 'sp-v1.2', 48, 2140, 386, 0, now() - interval '2 days', TRUE),
  ...
  (101, 'RISK',
   ...
   'meta-llama/llama-3.3-70b-instruct:free', 'rr-v2.0', 48, 3020, 512, 0, now() - interval '2 days', TRUE),
  ...
  (101, 'RECOMMENDATION',
   ...
   'meta-llama/llama-3.3-70b-instruct:free', 'rec-v1.4', 48, 2860, 448, 0, now() - interval '2 days', TRUE),

  -- 歷史版本：同 product + type，is_current = FALSE
  (101, 'SELLING_POINT',
   '{"points":[{"title":"抹茶口味","evidence":"多數評論正面","confidence":0.55}],"summary":"評論整體正面"}'::jsonb,
   'mistralai/mistral-7b-instruct:free', 'sp-v1.0', 31, 1420, 190, 0, now() - interval '35 days', FALSE),
  ...
  (104, 'RISK',
   ...
   'meta-llama/llama-3.3-70b-instruct:free', 'rr-v2.0', 124, 4820, 690, 0, now() - interval '2 days', TRUE),
  ...
  (104, 'RECOMMENDATION',
   ...
   'meta-llama/llama-3.3-70b-instruct:free', 'rec-v1.4', 124, 3980, 520, 0, now() - interval '2 days', TRUE),
  ...
  (107, 'TREND',
   ...
   'meta-llama/llama-3.3-70b-instruct:free', 'ti-v1.1', 0, 1980, 342, 0, now() - interval '2 days', TRUE),
  ...
  (113, 'SELLING_POINT',
   ...
   'meta-llama/llama-3.3-70b-instruct:free', 'sp-v1.2', 34, 1760, 298, 0, now() - interval '2 days', TRUE),
  ...
  (117, 'TREND',
   ...
   'meta-llama/llama-3.3-70b-instruct:free', 'ti-v1.1', 0, 1640, 286, 0, now() - interval '4 days', TRUE);
...
-- 另外補上 FR-10 提及的其他示警來源：熱度急墜、季節性不匹配、供應商異常。
-- 三種處理狀態都有，AC-10-2 的「已忽略不進預設清單」才測得到。
...
INSERT INTO risk_alert (product_id, risk_type, severity, trigger_value, status,
  ...
  (104, 'TOTAL_PENALTY',      'HIGH',   '扣分小計 24 分，達強制列入門檻 20 分',              'OPEN',        NULL, now() - interval '2 days', NULL, NULL),
  (116, 'REVIEW_RISK',        'MEDIUM', '負評率 18%（零食類門檻 15%）',                      'ACKNOWLEDGED', NULL, now() - interval '9 days', now() - interval '8 days', 2),
  (116, 'TOTAL_PENALTY',      'MEDIUM', '扣分小計 20 分，達強制列入門檻 20 分',              'ACKNOWLEDGED', NULL, now() - interval '9 days', now() - interval '8 days', 2),
  ...
  (114, 'HEAT_PLUNGE',        'LOW',    '7 日熱度斜率 −21%，30 日斜率 −38%',                 'IGNORED',
  ...
  (110, 'SUPPLIER_ANOMALY',   'LOW',    '供應商近 30 日交期延遲 2 次',                       'OPEN',        NULL, now() - interval '4 days', NULL, NULL);
